...
public void OnPluginStart()
{
	...
	// Return dudes to queue if failed
	HookEvent("game_rules_state_change", GameStateChange, EventHookMode_Pre)
...
public Action GameStateChange(Handle event, char[] name, bool dontBroadcast){
	...
	if(gameState == 7){
		...
		PrintToServer("il9 kek");
		// Players did not load, cancel match
	}
}
...
public void LoadMatchInfo()
{
	...
	ServerCommand("dota_force_gamemode %d", game_mode);
}
